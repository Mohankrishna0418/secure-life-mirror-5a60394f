...
const ServicePageTemplate = ({
  icon, badge, title, subtitle, description, features, cards, cardsTitle, cardsSubtitle, warranty = "5 Year Warranty",
}: ServicePageProps) => {
  return (
    <Layout>
      <section className="py-20 gradient-hero relative">
        <div className="container mx-auto px-4">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-2xl mx-auto">
            ...
            <h1 className="text-4xl md:text-5xl font-display font-bold text-primary-foreground mb-4">{title}</h1>
            <p className="text-primary-foreground/70 mb-6">{subtitle}</p>
            <Link to="/contact">
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-14 items-start">
            <motion.div initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <span className="text-primary font-medium text-xs uppercase tracking-widest mb-3 block">Features</span>
              <h2 className="text-3xl font-display font-bold text-foreground mb-4">Why Choose Us</h2>
            ...
            <motion.div initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={item.title} className="bg-card rounded-lg p-4 border border-border shadow-card text-center">
                  <div className="w-10 h-10 mx-auto gradient-primary rounded-md flex items-center justify-center mb-2">
                    <item.icon className="w-5 h-5 text-primary-foreground" />
                  </div>
                  <h4 className="font-display font-semibold text-foreground text-xs mb-0.5">{item.title}</h4>
      ...
      <section className="py-20 bg-muted">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center max-w-2xl mx-auto mb-12">
            <span className="text-primary font-medium text-xs uppercase tracking-widest mb-3 block">{cardsTitle}</span>
            <h2 className="text-3xl font-display font-bold text-foreground mb-3">{cardsSubtitle}</h2>
          </motion.div>
